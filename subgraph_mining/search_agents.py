...
def run_greedy_trial(args_tuple):
    ...
    # Simplified setup for a single trial
    ...
    while len(neigh) < args.max_pattern_size and frontier:
        ...
        with torch.no_grad():
            cand_embs = model.emb_model(utils.batch_nx_graphs(
                cand_neighs, anchors=anchors if args.node_anchored else None))
        ...
        # Save pattern if it meets the minimum size requirement
...
class GreedySearchAgent(SearchAgent):
    def __init__(self, min_pattern_size, max_pattern_size, model, dataset,
        ...
        model_type="order", out_batch_size=20, n_beams=1, n_workers=4): # Added n_workers
        """
        ...
        Args:
            ...
            n_workers: Number of CPU cores to use for parallel search trials.
        """
        super().__init__(min_pattern_size, max_pattern_size, model, dataset,
            embs, node_anchored=node_anchored, analyze=analyze,
            model_type=model_type, out_batch_size=out_batch_size)
        ...
        self.n_workers = n_workers # Store number of workers
    ...
    def run_search(self, n_trials=1000):
        """
        Overridden run_search to use multiprocessing for parallel trials.
        """
        self.cand_patterns = defaultdict(list)
        self.counts = defaultdict(lambda: defaultdict(list))
        self.n_trials = n_trials

        # Prepare arguments for each trial. The model and embeddings are passed to each
        # worker process. This is efficient with the 'fork' start method (default on Linux/macOS).
        # The `self.args` object should be attached to the agent before calling this.
        args_for_pool = [(i, self.model, self.dataset, self.embs, self.args) for i in range(n_trials)]
    ...
    def is_search_done(self):
        # This method is no longer directly used by the parallel run_search
        return True

    def step(self):
        # This method is now encapsulated within the `run_greedy_trial` worker function
        # and is not called directly in the parallel version.
        raise NotImplementedError("`step` is not used in the parallel GreedySearchAgent. The logic is in `run_greedy_trial`.")

    def init_search(self):
        # This method is not used in the parallel implementation.
        raise NotImplementedError("`init_search` is not used in the parallel GreedySearchAgent.")
    ...
    def finish_search(self):
        ...
        for pattern_size in range(self.min_pattern_size, self.max_pattern_size + 1):
            if self.rank_method == "hybrid":
                # Check if there are any counts available for the current size
                if self.counts[pattern_size]:
                    # The original code had a potential error here if values() is empty.
